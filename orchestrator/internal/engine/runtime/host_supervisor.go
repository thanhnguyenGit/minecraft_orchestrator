...
func NewHostSupervisor(ctx context.Context, config HostConfig, inbox *network.Inbox, bots []BotSpec) (*HostSupervisor, error) {
	...
	return &HostSupervisor{config: config, inbox: inbox, bots: append([]BotSpec(nil), bots...), allowed: allowed}, nil
}
...
func (s *HostSupervisor) Start(ctx context.Context) error {
	...
	s.logger().Info("mineflayer_host.started", "pid", command.Process.Pid, "script", script)
...
func (s *HostSupervisor) accept(ctx context.Context, listener net.Listener, token string) {
	...
	if err := s.serve(ctx, connection, token); err != nil && ctx.Err() == nil {
		s.logger().Error("mineflayer_host.socket_failed", "error", err)
	}
}
...
func (s *HostSupervisor) configure(connection net.Conn) error {
	...
	for _, bot := range s.bots {
		...
		bots = append(bots, &orchestratorv1.BotConfiguration{ProfileId: profile, Username: bot.Username, Host: s.config.Host, Port: uint32(s.config.Port), Auth: s.config.Auth, Version: s.config.Version})
	}
	payload, err := proto.Marshal(&orchestratorv1.HostEnvelope{Payload: &orchestratorv1.HostEnvelope_Configure{Configure: &orchestratorv1.HostConfigure{Generation: 1, Bots: bots}}})
	if err != nil {
		return err
	}
