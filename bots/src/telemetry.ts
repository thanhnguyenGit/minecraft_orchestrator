...
export function buildStateSnapshot(state: BotState): BotState {
  return {
    ...
    inventory: {
      selectedHotbarSlot: state.inventory.selectedHotbarSlot,
      slots: state.inventory.slots.map(({ slot, item }) => ({
        slot,
        ...(item ? { item: { ...item } } : {}),
      })),
    },
...
export function coalesceInventoryUpdates(
  updates: InventorySlot[],
): InventorySlot[] {
  const bySlot = new Map<number, InventorySlot>();
  for (const { slot, item } of updates)
    bySlot.set(slot, { slot, ...(item ? { item: { ...item } } : {}) });
  return [...bySlot.values()].sort((left, right) => left.slot - right.slot);
}

export function shouldEmitMotion(
  ...
  if (
    ...
    previous.velocityX === current.velocityX &&
    previous.velocityY === current.velocityY &&
    previous.velocityZ === current.velocityZ
  )
    return false;
